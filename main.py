...
def daily_entries_2_to_monthly():
    ...
    daily1 = pd.read_csv("dataflow.csv", skipinitialspace=True)
    daily2 = pd.read_csv("daily_2.csv", skipinitialspace=True)
    daily1.merge(daily2, on="Customer_Name", how="inner")
    daily_tot = pd.concat([daily1,daily2]).groupby(["Customer_Id","Customer_Name","Month", "Day"]).sum()
